...
class BatesCalibrator:
    ...
    def objective_function(self, params):
        """Compute sum of squared errors between model and market prices."""
        ...
        for i, strike in enumerate(self.strikes):
            ...
            paths = [
                self.engine.simulate_path(t_years=self.expiry)[-1]
                for _ in range(300)
            ]
            model_price = np.mean(np.maximum(np.array(paths) - strike, 0))
            errors.append((model_price - self.market_prices[i])**2)

        return np.sum(errors)

    def fit(self):
        """Run L-BFGS-B optimization and return calibrated parameters."""
        # Initial guesses: [kappa, theta, sigma_v, rho, lamb]
        init_guess = [2.0, 0.04, 0.3, -0.7, 0.1]
        bounds = [
            (0.1, 5.0), (0.01, 0.2), (0.1, 1.0), (-0.95, 0.0), (0.0, 1.0)
        ]

        print("Starting calibration... this may take a moment.")
        res = minimize(
            self.objective_function, init_guess,
